...
def is_horizontal_overlap(
    ...
    # key location
    x0, y0, w0, h0 = cv2.boundingRect(np.array(key_loc))
    x1, y1 = x0+w0, y0+h0
    # x0, y0 = key_loc[0]
    # x1, y1 = key_loc[2]
    ...
    # field location
    x, y, wf, hf = cv2.boundingRect(np.array(key_loc))
    x_, y_ = x + wf, y + hf
...
def compare_location(textline1, textline2):
    x1, y1, _, _ = cv2.boundingRect(np.array(textline1["location"]))
    x2, y2, _, _ = cv2.boundingRect(np.array(textline2["location"]))
    if is_horizontal_overlap(
        textline1["location"], textline2["location"], right_side=False, thres=0.8
    ):
        return x1 - x2
...
def group_pdf_files(image_list):
    """
    Group a list images into a list of pdf [list of pages]
    """
    image_name_list = np.unique(["_".join(image_path.split("\\")[-2].split("_")[:-1]) for image_path in image_list]).tolist()
    pdf_image = []
    for image_name in image_name_list:
        pdf_item = []
        for image_path in image_list:
            if image_name in image_path:
                pdf_item.append(image_path)
        
        pdf_image.append(pdf_item)
